  ...
  return (
    <View className="flex-1 bg-[#fdf8fa] pt-12">
      ...
      <ScrollView className="px-4" showsVerticalScrollIndicator={false}>
        {/* Notify */}
        <View className="bg-white rounded-3xl p-5 flex-row justify-between items-center mt-4 border border-[#e6dbe0] shadow-sm">
          <View>
            <View className="flex-row items-center gap-2">
              <Text className="text-base font-bold text-[#181114]">Notify Partners</Text>
              <MaterialIcons name="auto-awesome" size={18} color="#ee2b8c" />
            </View>
            <Text className="text-xs text-[#8c5f75] mt-0.5">Instantly share your desires with loved ones</Text>
          </View>
          <ToggleSwitch value={notifyPartners} onToggle={setNotifyPartners} />
        </View>

        {/* Categories */}
        <Text className="text-xl font-extrabold text-[#181114] tracking-tight mt-6 mb-3">{"Feeling..."}</Text>
            ...
            return (
              <TouchableOpacity key={c.id} className="items-center gap-2" onPress={() => setSelectedCat(c.id)}>
                <View className={`w-16 h-16 rounded-full items-center justify-center border-2 ${
                  sel ? "border-[#ee2b8c] bg-[#fce7f3]" : "border-[#e6dbe0] bg-white"
                }`}>
                  <MaterialIcons name={c.icon as any} size={28} color={sel ? "#ee2b8c" : "#8c5f75"} />
                </View>
                <Text className={`text-xs ${sel ? "font-bold text-[#ee2b8c]" : "font-semibold text-[#8c5f75]"}`}>{c.label}</Text>
              </TouchableOpacity>
            );
          })}
        </ScrollView>
        ...
        <View className="flex-row justify-between items-center mt-4">
          <Text className="text-xl font-extrabold text-[#181114] tracking-tight">{"Today's Desires"}</Text>
          <TouchableOpacity onPress={() => setAddedItems([])}><Text className="text-sm font-bold text-[#ee2b8c]">Clear all</Text></TouchableOpacity>
        </View>
        ...
        <View className="flex-row flex-wrap gap-4 mt-3">
          {cravingsData.map((item) => {
            ...
            return (
              <View key={item.id} className="bg-white rounded-2xl overflow-hidden border border-[#e6dbe0]" style={{ width: "47%" } as any}>
                <View className="w-full h-36 bg-[#f5f0f2] items-center justify-center relative">
                ...
                <Text className="text-base font-bold text-[#181114] px-3 pt-3">{item.name}</Text>
                <Text className="text-xs text-[#8c5f75] px-3 pt-0.5">{item.desc}</Text>
                <TouchableOpacity
                  className={`flex-row items-center justify-center gap-1 mx-3 my-3 rounded-full py-2 ${
                    added ? "bg-[#22c55e]" : "bg-[#ee2b8c]"
                  }`}
        ...
        {/* Custom */}
        <TouchableOpacity
          className="flex-row items-center justify-center gap-2 bg-[#fce7f3] rounded-full py-4 mt-6 border-2 border-dashed border-[#ee2b8c]"
          onPress={() => router.push("/add-craving")}
        >
          <MaterialIcons name="add-circle-outline" size={22} color="#ee2b8c" />
          <Text className="text-base font-bold text-[#ee2b8c]">Add Custom Craving</Text>
        </TouchableOpacity>
      ...
      {toast && (
        <View className="absolute bottom-8 left-4 right-4 flex-row items-center gap-2 bg-[rgba(24,17,20,0.9)] rounded-full px-5 py-3">
